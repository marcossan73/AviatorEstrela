#!/bin/bash
# Instalação forçada do Chrome e ChromeDriver

set -e

echo "=== INSTALAÇÃO FORÇADA DO CHROME ==="
echo ""

# 1. Remover instalações antigas
echo "1. Removendo instalações antigas..."
...
# 4. Adicionar repositório do Chrome
echo ""
echo "4. Adicionando repositório oficial do Chrome..."
wget -q -O - https://dl-ssl.google.com/linux/linux_signing_key.pub | sudo apt-key add -
sudo sh -c 'echo "deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main" >> /etc/apt/sources.list.d/google-chrome.list'

# 5. Instalar Chrome via apt (método mais confiável)
echo ""
...
if command -v google-chrome-stable &>/dev/null; then
    ...
    echo "    Versão: $CHROME_VER"
else
    echo "  ? ERRO: Chrome não foi instalado corretamente"
    exit 1
fi

# 7. Criar link simbólico se necessário
echo ""
echo "7. Criando links simbólicos..."
...
echo "  Chrome versão: $CHROME_VERSION (major: $CHROME_MAJOR)"
...
if [ "$CHROME_MAJOR" -ge 115 ]; then
    ...
    if wget -q "$DRIVER_URL" 2>/dev/null; then
        echo "  ? Download bem-sucedido (versão específica)"
    else
        # Fallback: pegar versão LATEST conhecida
        echo "  Versão específica não disponível, usando versão estável..."
...
if command -v chromedriver &>/dev/null; then
    ...
    echo "  ? ERRO: ChromeDriver não foi instalado"
    exit 1
fi
...
if google-chrome-stable --headless --disable-gpu --dump-dom https://www.google.com 2>&1 | grep -q "Google"; then
    ...
    echo "  ? Teste falhou, mas instalação concluída"
fi

# 11. Instalar Xvfb (necessário para headless em alguns casos)
echo ""
...
echo "  Versão: $(google-chrome-stable --version)"
echo "  Driver: $(which chromedriver)"
echo "  Versão Driver: $(chromedriver --version)"
echo ""
